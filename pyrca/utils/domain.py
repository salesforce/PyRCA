...
class DomainParser:
    ...
    def get_require_links(self):
        return None if self.config is None or "causal-graph" not in self.config \
    ...
    def get_metrics(self):
        return None if self.config is None or "metrics" not in self.config \
            else self.config["metrics"]
